...
export function toMockProduct(p: ProductWithDetails): Product {
  ...
  p.variants.forEach((v) => {
    if (v.is_active) sizeSet.add(v.size);
  });
  const sizes = [...sizeSet];
  ...
  p.variants.forEach((v) => {
    if (v.is_active) colorSet.add(v.color);
  });
  ...
  const stock = p.variants.filter((v) => v.is_active).reduce((sum, v) => sum + v.stock, 0);
  ...
  p.variants
    ...
    .forEach((v) => {
      stockByVariant[`${v.size}-${v.color}`] = v.stock;
    });
