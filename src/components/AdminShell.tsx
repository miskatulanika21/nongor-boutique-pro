...
export function AdminShell({ children }: { children: ReactNode }) {
    ...
    <div className="bg-sidebar text-sidebar-foreground h-full flex flex-col">
      <div className="p-5 flex items-center justify-between border-b border-sidebar-border">
        ...
        <button
          ...
          className="md:hidden p-1.5 rounded-full hover:bg-sidebar-accent"
        >
          <X className="h-5 w-5" />
        </button>
      </div>
      <nav className="flex-1 px-3 py-4 space-y-5 overflow-y-auto scrollbar-none">
        {groups.map((g) => {
          ...
          return (
            <div key={g}>
              <div className="px-3 mb-1.5 text-[10px] uppercase tracking-[0.2em] text-sidebar-foreground/45 font-semibold">
                {g}
              </div>
              <div className="space-y-0.5">
                {items.map((l) => {
                  ...
                  return (
                    ...
                    >
                      <span
                        className={`relative grid place-items-center h-7 w-7 rounded-md transition ${isActive ? "bg-sidebar-primary/15 text-sidebar-primary" : "text-sidebar-foreground/60 group-hover:text-sidebar-foreground"}`}
                      >
      ...
      <div className="p-4 border-t border-sidebar-border space-y-2">
        <Link
          to="/"
          className="flex items-center gap-2 text-xs text-sidebar-foreground/65 hover:text-sidebar-primary transition"
        >
  ...
  return (
    <div className="min-h-screen bg-secondary/60 flex">
      <aside className="hidden md:block w-64 shrink-0 fixed inset-y-0 left-0 border-r border-sidebar-border">
        {Sidebar}
      </aside>
      {mobileOpen && (
        <div className="md:hidden fixed inset-0 z-50">
          <div
            className="absolute inset-0 bg-charcoal/60 backdrop-blur-sm animate-fade-up"
            onClick={() => setMobileOpen(false)}
          />
          <aside className="absolute left-0 top-0 bottom-0 w-72 shadow-elegant animate-fade-up">
            {Sidebar}
          </aside>
        </div>
      )}
      <div className="flex-1 md:ml-64 min-w-0">
        <header className="bg-ivory/90 backdrop-blur-xl border-b border-hairline sticky top-0 z-30">
          <div className="flex items-center gap-3 px-4 md:px-8 h-16">
            <button
              className="md:hidden p-2 rounded-full hover:bg-cream"
            ...
            <div className="ml-auto flex items-center gap-2">
              <div className="hidden md:flex items-center bg-cream/60 border border-hairline rounded-full pl-3 pr-2 w-72 transition focus-within:border-gold focus-within:bg-ivory">
                ...
                <input
                  ...
                  className="bg-transparent border-0 outline-none h-9 px-2 text-sm flex-1"
                />
                <kbd className="text-[10px] text-muted-foreground px-1.5 py-0.5 rounded bg-secondary border border-hairline">
                  ⌘K
                </kbd>
              </div>
              <button
                className="h-10 w-10 grid place-items-center rounded-full hover:bg-cream transition relative"
              ...
              <button className="flex items-center gap-2 pl-1 pr-2 py-1 rounded-full hover:bg-cream transition">
                <div className="h-8 w-8 rounded-full bg-gradient-to-br from-maroon to-maroon-deep text-primary-foreground grid place-items-center text-xs font-semibold shadow-soft">
                  {adminDisplay.initial}
                </div>
