...
export async function adminCreateCategory(
  ...
  if (error) { console.error('[categories] create error:', error); return null; }
  return data;
}
...
export async function adminUpdateCategory(
  ...
  if (error) { console.error('[categories] update error:', error); return null; }
  return data;
}

export async function adminDeleteCategory(id: string): Promise<boolean> {
  const { error } = await supabase.from('categories').delete().eq('id', id);
  if (error) { console.error('[categories] delete error:', error); return false; }
  return true;
}
