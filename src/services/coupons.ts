...
export async function validateCoupon(code: string, orderTotal: number): Promise<{
  ...
  if (!isSupabaseConfigured) {
    ...
    if (orderTotal < m.minOrder) return { valid: false, error: `Minimum order ৳${m.minOrder} required` };
...
export async function adminGetAllCoupons(): Promise<Coupon[]> {
  ...
  if (error) { console.error('[coupons] adminGetAll error:', error); return []; }
  return data ?? [];
}
...
export async function adminCreateCoupon(coupon: InsertTables<'coupons'>): Promise<Coupon | null> {
  ...
  if (error) { console.error('[coupons] create error:', error); return null; }
  return data;
}

export async function adminUpdateCoupon(id: string, patch: UpdateTables<'coupons'>): Promise<Coupon | null> {
  const { data, error } = await supabase
    .from('coupons')
    .update(patch)
    .eq('id', id)
  ...
  if (error) { console.error('[coupons] update error:', error); return null; }
  return data;
}

export async function adminDeleteCoupon(id: string): Promise<boolean> {
  const { error } = await supabase.from('coupons').delete().eq('id', id);
  if (error) { console.error('[coupons] delete error:', error); return false; }
  return true;
}
