...
export async function adminGetAllPayments(): Promise<
  (Payment & { order?: { order_number: string; customer_name: string } })[]
> {
 ...
 */
export async function uploadPaymentProof(_orderId: string, _file: File): Promise<string | null> {
  console.warn(
    "[payments] Payment proof upload is disabled in Phase 1. Using TrxID-only verification.",
  );
  return null;
}
 ...
 */
export async function adminUploadPaymentProof(orderId: string, file: File): Promise<string | null> {
  ...
  await supabase.from("payments").update({ proof_image_url: path }).eq("order_id", orderId);
...
export async function adminGetProofUrl(path: string): Promise<string | null> {
  ...
  const { data, error } = await supabase.storage.from("payment-proofs").createSignedUrl(path, 300); // 5 minute expiry
