    ...
    orders
      .filter(
        (o) =>
          o.date === todayStr &&
          o.status !== "Cancelled" &&
          (o.paymentStatus === "Paid" || o.paymentStatus === "COD"),
      )
    ...
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
        ...
        <StatCard
          ...
          delta={`${pendingNeedsVerification} need verification`}
          accent={pendingCount > 0 ? "warn" : "good"}
        />
        <StatCard
          label="Customers"
          value={String(totalUniqueCustomers)}
          delta="+24 this week"
          accent="good"
        />
      </div>
              ...
              <AreaChart data={chartData}>
                <defs>
                  <linearGradient id="g" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="#6b1f2a" stopOpacity={0.5} />
                    <stop offset="100%" stopColor="#6b1f2a" stopOpacity={0} />
                  </linearGradient>
                </defs>
        ...
        <div className="bg-card rounded-xl p-5 border border-border/60">
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
                  data={dynamicOrderStatusChart}
                  dataKey="value"
                  nameKey="name"
                  innerRadius={50}
                  outerRadius={80}
                >
                  {dynamicOrderStatusChart.map((_, i) => (
      ...
      <div className="grid lg:grid-cols-[1fr_320px] gap-4 mt-6">
        <div className="bg-card rounded-xl border border-border/60">
          <div className="p-5 border-b border-border/60 flex items-center justify-between">
            ...
            <Link to="/admin/orders" className="text-xs text-maroon">
              View all →
            </Link>
          </div>
            ...
            <tbody>
              {recentOrders.map((o) => (
                <tr key={o.id} className="border-t border-border/40">
                  ...
                  <td className="p-3">{taka(o.total)}</td>
                  <td className="p-3">
        ...
        <div className="space-y-4">
          <div className="bg-card rounded-xl p-5 border border-border/60">
            ...
            <div className="grid grid-cols-2 gap-2">
              <Link
                to="/admin/products"
                className="bg-maroon text-primary-foreground rounded-lg p-3 text-xs font-semibold flex flex-col items-center gap-1"
              >
              ...
              <Link
                to="/admin/orders"
                className="bg-secondary rounded-lg p-3 text-xs font-semibold flex flex-col items-center gap-1"
              >
          ...
          <div className="bg-card rounded-xl p-5 border border-border/60">
            <h3 className="font-display text-base mb-3">
              Best Sellers (Low Stock: {lowStockCount})
            </h3>
              ...
              {bestSellers.map((p) => (
                <div key={p.id} className="flex gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-maroon">{taka(p.discountPrice ?? p.price)}</div>
                  </div>
                </div>
              ))}
            </div>
