  ...
  return (
    <div>
      <PageHeader title="Products" subtitle={`${products.length} products`} actions={
        <button onClick={openNew} className="bg-maroon text-primary-foreground rounded-lg px-4 py-2 text-sm font-semibold flex items-center gap-1"><Plus className="h-4 w-4" /> Add Product</button>
      } />
      <TableShell toolbar={
        <>
          ...
          <tbody>
            {filtered.map((p) => (
              <tr key={p.id} className="border-t border-border/40">
                ...
                <td className="p-3 text-right"><button onClick={() => openEdit(p)} className="p-1 hover:text-maroon"><Edit className="h-4 w-4" /></button><button onClick={() => handleDelete(p.id)} className="p-1 hover:text-destructive"><Trash2 className="h-4 w-4" /></button></td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
          <DialogTitle className="font-display text-2xl">{editProduct ? "Edit Product" : "Add Product"}</DialogTitle>
          <ProductForm
            initial={editProduct}
            onSave={(data) => {
              if (editProduct) {
...
function ProductForm({ initial, onSave }: { initial: Product | null; onSave: (data: Partial<Product>) => void }) {
  ...
  return (
    <div className="grid md:grid-cols-2 gap-3 mt-4 text-sm">
      <div><label className="text-xs text-muted-foreground">Product name</label><input value={name} onChange={(e) => setName(e.target.value)} className="mt-1 w-full px-3 py-2 rounded-lg bg-secondary text-sm" /></div>
      <div><label className="text-xs text-muted-foreground">Slug</label><input value={slug} onChange={(e) => setSlug(e.target.value)} placeholder="auto-generated" className="mt-1 w-full px-3 py-2 rounded-lg bg-secondary text-sm" /></div>
      <div><label className="text-xs text-muted-foreground">Price</label><input type="number" value={price} onChange={(e) => setPrice(e.target.value)} className="mt-1 w-full px-3 py-2 rounded-lg bg-secondary text-sm" /></div>
      <div><label className="text-xs text-muted-foreground">Discount price</label><input type="number" value={discountPrice} onChange={(e) => setDiscountPrice(e.target.value)} className="mt-1 w-full px-3 py-2 rounded-lg bg-secondary text-sm" /></div>
      <div><label className="text-xs text-muted-foreground">Category</label><input value={category} onChange={(e) => setCategory(e.target.value)} className="mt-1 w-full px-3 py-2 rounded-lg bg-secondary text-sm" /></div>
      ...
      <div className="md:col-span-2 flex gap-4 text-sm">
        ...
        <label className="flex items-center gap-2"><input type="checkbox" checked={isBest} onChange={(e) => setIsBest(e.target.checked)} className="accent-maroon" /> Best Seller</label>
      </div>
      <div className="md:col-span-2 flex justify-end gap-2 pt-3">
        <DialogClose className="px-4 py-2 rounded-lg border border-border text-sm">Cancel</DialogClose>
        <button onClick={submit} className="px-5 py-2 rounded-lg bg-maroon text-primary-foreground text-sm font-semibold">
