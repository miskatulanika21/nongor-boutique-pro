...
export const Route = createFileRoute("/_shop/shop")({
  ...
  head: () => ({
    meta: [
      { title: "Shop Handmade Kurti — Nongor" },
      { name: "description", content: "Browse our handmade kurti collection." },
    ],
  }),
  component: Shop,
});
...
function Shop() {
  ...
  const filtered = useMemo(() => {
    ...
    let r = products.filter((p) => {
      ...
      if (term) {
        const hay =
          `${p.name} ${p.fabric} ${p.colors.map((c) => c.name).join(" ")} ${p.collection.join(" ")}`.toLowerCase();
        if (!hay.includes(term)) return false;
      }
      return true;
    });
    if (sort === "low")
      r = r.sort((a, b) => (a.discountPrice ?? a.price) - (b.discountPrice ?? b.price));
    if (sort === "high")
      r = r.sort((a, b) => (b.discountPrice ?? b.price) - (a.discountPrice ?? a.price));
    if (sort === "best") r = r.sort((a, b) => b.reviewCount - a.reviewCount);
    return r;
  }, [products, sort, size, color, fabric, collection, priceMax, q]);
  ...
  const activePills: { label: string; clear: () => void }[] = [
    ...
    ...collection.map((c) => ({
      label: c,
      clear: () => setCollection(collection.filter((x) => x !== c)),
    })),
    ...
    <div className="space-y-6 text-sm">
      <FilterGroup title="Collection">
        {collections.map((c) => (
          <Check
            key={c}
            label={c}
            checked={collection.includes(c)}
            onChange={() => toggle(collection, setCollection, c)}
          />
        ))}
      </FilterGroup>
        ...
        <div className="flex flex-wrap gap-2">
          {allSizes.map((s) => (
            <button
              ...
              className={`h-9 w-9 rounded-md border text-xs font-medium transition ${size.includes(s) ? "bg-maroon text-primary-foreground border-maroon" : "border-border hover:border-maroon"}`}
            >
              {s}
            </button>
          ))}
      ...
      <FilterGroup title="Color">
        {allColors.map((c) => (
          <Check
            ...
            onChange={() => toggle(color, setColor, c)}
          />
        ))}
      </FilterGroup>
      <FilterGroup title="Fabric">
        {allFabrics.map((c) => (
          <Check
            key={c}
            label={c}
            checked={fabric.includes(c)}
            onChange={() => toggle(fabric, setFabric, c)}
          />
        ))}
      </FilterGroup>
      <FilterGroup title="Price">
        ...
        <input
          ...
          className="w-full accent-maroon mt-2"
        />
      </FilterGroup>
    </div>
  );
      ...
      <div>
        ...
        <h1 className="mt-2 font-display text-3xl md:text-5xl">
          {q ? (
            ...
            "Handmade Kurti Collection"
          )}
        </h1>
        <p className="mt-2 text-muted-foreground text-sm md:text-base">
          {filtered.length} pieces · hand-stitched in Bangladesh
        </p>
        ...
        {activePills.length > 0 && (
          <div className="mt-4 flex flex-wrap items-center gap-2">
            ...
            <button
              onClick={clearAll}
              className="text-xs text-muted-foreground hover:text-maroon underline-offset-2 hover:underline ml-1"
            >
              Clear all
            </button>
          </div>
        ...
        <div>
            ...
            <div className="flex items-center gap-2 ml-auto">
              <div className="hidden md:flex items-center bg-card rounded-full border border-border p-1">
                <button
                  ...
                  className={`h-8 w-8 grid place-items-center rounded-full ${view === "grid" ? "bg-maroon text-primary-foreground" : ""}`}
                >
                  <Grid3x3 className="h-4 w-4" />
                </button>
                <button
                  onClick={() => setView("list")}
                  className={`h-8 w-8 grid place-items-center rounded-full ${view === "list" ? "bg-maroon text-primary-foreground" : ""}`}
                >
              ...
              <div className="relative">
                <select
                  ...
                  className="appearance-none bg-card border border-border rounded-full pl-4 pr-9 py-2 text-sm"
                >
          ...
          {productsLoading ? (
            ...
            <div className="bg-card rounded-2xl p-16 text-center">
              ...
              <button
                onClick={clearAll}
                className="mt-4 text-sm text-maroon underline underline-offset-4"
              >
                Reset filters
              </button>
            </div>
            ...
            <div className="space-y-4">
              {filtered.map((p) => (
                <div key={p.id} className="bg-card rounded-2xl p-4 flex gap-4 shadow-soft">
                  <img
                    src={p.images[0]}
                    className="h-32 w-24 md:h-40 md:w-32 object-cover rounded-lg"
                    alt={p.name}
                  />
                  <div className="flex-1">
                    ...
                    <div className="mt-3 font-display text-lg text-maroon">
